...
# capture_running = True  # 동영상 캡처 상태를 추적하는 변수
# info_displayed = False  # 정보 출력 여부를 추적하는 변수
# game_running = True  # 게임 루프가 계속 실행되는지 여부를 추적하는 변수
...
# def process_frame():
#     global frame, countdown_done, info_displayed, game_running
#     countdown_start_time = None
#     countdown_number = None
#     CA = ""
    
#     while game_running:  # 게임이 계속 실행될지 여부를 추적
#         countdown_done = False  # 카운트다운 초기화
#         info_displayed = False  # 정보 출력 초기화
#         countdown_start_time = None  # 카운트다운 시간 초기화
#         countdown_number = None
#         CA = ""
#         max_label = "None"  # YOLO가 인식한 결과를 저장
...
#                         countdown_done = True  # 카운트다운 완료
#                         info_displayed = True  # 정보 출력 플래그 설정
...
#                 elif countdown_done:
#                     # 컴퓨터의 응답 및 내 응답 표시
#                     cv2.putText(frame, CA, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
#                     cv2.putText(frame, "YOU - " + str(max_label), (350, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
#                     answer = winOrNot(computer_Answer, max_label)
#                     print("Result:", answer)
#                     cv2.putText(frame, answer, (200, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
info_displayed = False  # 정보 출력 여부를 추적하는 변수
game_running = True  # 게임 루프가 계속 실행되는지 여부를 추적하는 변수
max_label = "None"  # 실시간으로 감지된 사용자의 동작 저장
...
def process_frame():
    global frame, countdown_done, info_displayed, game_running, max_label
    countdown_start_time = None
    countdown_number = None
    CA = ""
    ...
    while game_running:  # 게임이 계속 실행될지 여부를 추적
        ...
        while True:
            with lock:
                ...
                # 실시간 감지된 동작과 카운트다운 숫자 또는 결과 메시지 표시
                if countdown_number is not None:
                    cv2.putText(frame, str(countdown_number), (frame.shape[1] // 2 - 50, frame.shape[0] // 2), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                elif countdown_done:
                    # 컴퓨터의 응답 및 내 응답 표시
                    ...
                    # 게임 결과를 3초 동안 화면에 유지
                    cv2.imshow("Gesture Detection", frame)
                    cv2.waitKey(3000)  # 3초 동안 결과를 표시

                    # 게임 종료를 위한 루프 종료 준비
                    break

                # 실시간으로 YOLO가 감지한 내 동작 표시
                cv2.putText(frame, "Your Move: " + str(max_label), (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
